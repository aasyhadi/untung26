		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						@if(ucc())
				   		{{Html::btnModal('<i class="la la-plus-circle"></i> Tambah Modul','modal-tambah','primary')}}
				   		<hr>
				   		@endif
...
{{ Form::bsOpen('form-tambah', url($main_path."/insert"), 'POST', ['enctype' => 'multipart/form-data']) }}
    {{ Html::mOpenLG('modal-tambah', 'Tambah Modul') }}
    <div class="row">
        <!-- Kolom Kiri -->
        <div class="col-md-6">
            {{ Form::bsTextField('Nama Modul PPT', 'nama_modul', '', true, 'md-12') }}
            {{ Form::bsNumeric('Harga', 'harga', '', false, 'md-12') }}
            <!-- {{ Form::bsTextField('Modul Link', 'modul_link', '', false, 'md-12') }} -->
...
{{ Form::bsOpen('form-edit',url($main_path."/update")) }}
	{{Html::mOpenLG('modal-edit','Edit Modul')}}
        <div class="row">
            <!-- Kolom Kiri -->
            <div class="col-md-6">
                {{ Form::bsTextField('Nama Modul PPT', 'nama_modul', '', true, 'md-12') }}
...
<script type="text/javascript">
	$(function(){
		var $tabel1 = $('#datatable').DataTable({
		    ...
		    fixedHeader: true,
		    serverSide: true,
		    ajax: "{{url('master-modul-ppt/dt')}}",
		    "iDisplayLength": 10,
		    columns: [
		    	 {data:'DT_RowIndex' , orderable:false, searchable: false,sClass:""},
		         {data:'nama_modul' , name:"nama_modul" , orderable:true, searchable: true,sClass:""},
		...
		$("#modal-edit").on('show.bs.modal', function(e){
			...
			$.get("{{url('master-modul-ppt/get-data')}}/"+$uuid, function(respon){
				if(respon.status){
					$('#form-edit #uuid').val(respon.data.uuid);
					$('#form-edit #nama_modul').val(respon.data.nama_modul);
